...
//    [self.heightTextField setBorderStyle:UITextBorderStyleNone];
    ...
    UILabel *tipLabel =  [UIFactory createLabelWith:textFieldFrame
                                     text:@"完善个人信息，你会有更多好友哦~"
...
- (void)onDone
{
    ...
    //保存头像
    if (self.headImage) {
        [self saveHeadImg:self.headImage toPath:[UIFactory getAvatarFilePath]];
    }
    
    NSDateFormatter *tempDateFormatter = [[NSDateFormatter alloc] init];
    [tempDateFormatter setDateFormat:@"yyyyMMdd"];
    ...
    [bodyParams setValue:[NSNumber numberWithInt:self.sex] forKey:@"sex"];
    [bodyParams setValue:[NSNumber numberWithInt:self.height] forKey:@"height"];
    [bodyParams setValue:[NSNumber numberWithInt:self.weight * 10] forKey:@"weight"];
    [bodyParams setValue:[tempDateFormatter stringFromDate:self.birthday] forKey:@"birth"];
